...
func (data DeviceAdminAuthorizationRuleUpdateRanks) toBody(ctx context.Context, state DeviceAdminAuthorizationRuleUpdateRanks) string {
	body := ""
	if len(data.Rules) > 0 {
...
func (data *DeviceAdminAuthorizationRuleUpdateRanks) fromBody(ctx context.Context, res gjson.Result) {
	if value := res.Get("response.rules"); value.Exists() {
		data.Rules = make([]DeviceAdminAuthorizationRuleUpdateRanksRules, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := DeviceAdminAuthorizationRuleUpdateRanksRules{}
...
func (data *DeviceAdminAuthorizationRuleUpdateRanks) isNull(ctx context.Context, res gjson.Result) bool {
	if len(data.Rules) > 0 {
		return false
	}
	return true
