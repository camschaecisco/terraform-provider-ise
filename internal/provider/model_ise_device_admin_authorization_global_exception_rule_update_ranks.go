...
func (data DeviceAdminAuthorizationGlobalExceptionRuleUpdateRanks) toBody(ctx context.Context, state DeviceAdminAuthorizationGlobalExceptionRuleUpdateRanks) string {
	body := ""
	if len(data.Rules) > 0 {
...
func (data *DeviceAdminAuthorizationGlobalExceptionRuleUpdateRanks) fromBody(ctx context.Context, res gjson.Result) {
	if value := res.Get("response.rules"); value.Exists() {
		data.Rules = make([]DeviceAdminAuthorizationGlobalExceptionRuleUpdateRanksRules, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := DeviceAdminAuthorizationGlobalExceptionRuleUpdateRanksRules{}
...
func (data *DeviceAdminAuthorizationGlobalExceptionRuleUpdateRanks) isNull(ctx context.Context, res gjson.Result) bool {
	if len(data.Rules) > 0 {
		return false
	}
	return true
