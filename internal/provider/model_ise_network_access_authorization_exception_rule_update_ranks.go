...
type NetworkAccessAuthorizationExceptionRuleUpdateRanks struct {
	Id          types.String                                              `tfsdk:"id"`
	PolicySetId types.String                                              `tfsdk:"policy_set_id"`
	Rules       []NetworkAccessAuthorizationExceptionRuleUpdateRanksRules `tfsdk:"rules"`
}

type NetworkAccessAuthorizationExceptionRuleUpdateRanksRules struct {
	Id   types.String `tfsdk:"id"`
...
func (data NetworkAccessAuthorizationExceptionRuleUpdateRanks) toBody(ctx context.Context, state NetworkAccessAuthorizationExceptionRuleUpdateRanks) string {
	body := ""
	if len(data.Rules) > 0 {
...
func (data *NetworkAccessAuthorizationExceptionRuleUpdateRanks) fromBody(ctx context.Context, res gjson.Result) {
	if value := res.Get("response.rules"); value.Exists() {
		data.Rules = make([]NetworkAccessAuthorizationExceptionRuleUpdateRanksRules, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := NetworkAccessAuthorizationExceptionRuleUpdateRanksRules{}
...
func (data *NetworkAccessAuthorizationExceptionRuleUpdateRanks) isNull(ctx context.Context, res gjson.Result) bool {
	if len(data.Rules) > 0 {
		return false
	}
	return true
