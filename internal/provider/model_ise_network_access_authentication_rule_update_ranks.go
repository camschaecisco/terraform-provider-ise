...
func (data NetworkAccessAuthenticationRuleUpdateRanks) toBody(ctx context.Context, state NetworkAccessAuthenticationRuleUpdateRanks) string {
	body := ""
	if len(data.Rules) > 0 {
...
func (data *NetworkAccessAuthenticationRuleUpdateRanks) fromBody(ctx context.Context, res gjson.Result) {
	if value := res.Get("response.rules"); value.Exists() {
		data.Rules = make([]NetworkAccessAuthenticationRuleUpdateRanksRules, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := NetworkAccessAuthenticationRuleUpdateRanksRules{}
...
func (data *NetworkAccessAuthenticationRuleUpdateRanks) isNull(ctx context.Context, res gjson.Result) bool {
	if len(data.Rules) > 0 {
		return false
	}
	return true
