...
type DeviceAdminAuthorizationExceptionRuleUpdateRanks struct {
	Id          types.String                                            `tfsdk:"id"`
	PolicySetId types.String                                            `tfsdk:"policy_set_id"`
	Rules       []DeviceAdminAuthorizationExceptionRuleUpdateRanksRules `tfsdk:"rules"`
}

type DeviceAdminAuthorizationExceptionRuleUpdateRanksRules struct {
	Id   types.String `tfsdk:"id"`
...
func (data DeviceAdminAuthorizationExceptionRuleUpdateRanks) toBody(ctx context.Context, state DeviceAdminAuthorizationExceptionRuleUpdateRanks) string {
	body := ""
	if len(data.Rules) > 0 {
...
func (data *DeviceAdminAuthorizationExceptionRuleUpdateRanks) fromBody(ctx context.Context, res gjson.Result) {
	if value := res.Get("response.rules"); value.Exists() {
		data.Rules = make([]DeviceAdminAuthorizationExceptionRuleUpdateRanksRules, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := DeviceAdminAuthorizationExceptionRuleUpdateRanksRules{}
...
func (data *DeviceAdminAuthorizationExceptionRuleUpdateRanks) isNull(ctx context.Context, res gjson.Result) bool {
	if len(data.Rules) > 0 {
		return false
	}
	return true
